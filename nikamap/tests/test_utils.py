...
def test_pos_uniform():
    ...
    assert -0.5 < x.min() or x.max() < shape[1] - \
        0.5, 'pixel coordinate outside boundaries'
    assert -0.5 < y.min() or y.max() < shape[0] - \
        0.5, 'pixel coordinate outside boundaries'
    ...
    assert shape[1] * 0.5 - \
        1 < np.floor(x.min() + 0.5), 'pixel coordinate outside within'
    assert shape[0] * 0.5 - \
        1 < np.floor(y.min() + 0.5), 'pixel coordinate outside within'
...
def test_pos_gridded():
    ...
    assert np.all(x.reshape(3, 3) == np.linspace(
        shape[1] / 4, shape[1] * 3 / 4, 3) - 0.5), 'unexpected pixel coordinate'
    assert np.all(y.reshape(3, 3).T == np.linspace(
        shape[0] / 4, shape[0] * 3 / 4, 3) - 0.5), 'unexpected pixel coordinate'
...
def test_pos_list():
    ...
    with pytest.warns(UserWarning):
        x, y = pos_list(nsources=nsources, shape=shape,
                        mask=mask, x_mean=x_mean, y_mean=y_mean)
...
def test_fft_2D_hanning():
    ...
    unchanged = slice(((shape - 1) - (size - 1) + (apod_size * 2 + 1 - 1)) //
                      2, ((shape - 1) + (size - 1) - (apod_size * 2 + 1 - 1)) // 2 + 1)
